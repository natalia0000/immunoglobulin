...
def visual(visual_flag):
    ...
    print(seq_len_c)
    if visual_flag:
        seaborn.distplot(seq_len, axlabel='Length of sequences', bins=20)
        plt.show()
...
#add arguments
    ...
    parser.add_argument('--p_compl', help='Probability of two nucleotides joining complementary strands', default=0.8,
                        type=float)
    # p_compl = 0.8  # вероятность того, что хватит двух комплементарных нуклеотидов подряд, чтобы соединить цепочки
    parser.add_argument('--p_number_seq', help='Probability of nucleotides joining to the first strand', default=0.5,
                        type=float)
